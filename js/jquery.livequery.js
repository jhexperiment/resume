...
(function(factory) {
  ...
  function _match(me, query, fn, fn2) {
    return me.selector == query.selector && me.context == query.context && (!fn || fn.$lqguid == query.fn.$lqguid) && (!fn2 || fn2.$lqguid == query.fn2.$lqguid);
  }
  ...
  $.extend($.fn, {
    ...
    expire: function(fn, fn2) {
      ...
      $.each($jQlq.queries, function(i, query) {
        if (_match(me, query, fn, fn2) && !me.stopped) $jQlq.stop(query.id);
      });
  ...
  $.extend($jQlq, {
    ...
    run: function(id) {
      if (id !== undefined) {
        ...
        if ($.inArray(id, $jQlq.queue) < 0) $jQlq.queue.push(id);
      } else
